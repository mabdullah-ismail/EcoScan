...
// --- PDF Generation Shared Functions ---

const downloadBatchCertificate = (items) => {
    ...
    doc.text('BATCH SITE REPORT', W/2, 38, { align:'center' });

    // Summary
    doc.setTextColor(30,30,30); doc.setFontSize(11); doc.setFont('helvetica','normal');
    doc.text(`Date: ${date}`, 15, 58);
    doc.text(`Site ID: ${getSessionId()}`, 15, 65);
    doc.text(`Total Materials Scanned: ${items.length}`, 15, 72);

    let totalCarbonSaving = 0;
    let totalCostSaving = 0;
    items.forEach(item => {
        const carbon = item.resultData.carbon || 0.5;
        const altCarbon = item.resultData.alt_carbon || 0.2;
        totalCarbonSaving += (carbon - altCarbon);
        totalCostSaving += (item.resultData.cost * ((item.resultData.cost_saving_pct || 35) / 100));
    });

    doc.setFontSize(14); doc.setFont('helvetica','bold'); doc.setTextColor(94,125,107);
    doc.text('ESTIMATED SITE IMPACT', 15, 86);
    doc.setFillColor(240,247,243); doc.roundedRect(15, 92, 180, 25, 3, 3, 'F');
    doc.setTextColor(30,30,30); doc.setFontSize(11);
    doc.text(`Estimated CO\u2082 Reduction: ${totalCarbonSaving.toFixed(2)} kg`, 25, 100);
    doc.text(`Estimated Cost Benefit: PKR ${totalCostSaving.toLocaleString()}`, 25, 108);

    // List
    doc.text('MATERIAL BREAKDOWN', 15, 130);
    doc.setDrawColor(94,125,107); doc.line(15, 133, W-15, 133);
    
    let y = 142;
    items.forEach((item, i) => {
        if (y > 250) { doc.addPage(); y = 20; }
        doc.setFontSize(10); doc.setFont('helvetica','bold');
        doc.text(`${i+1}. ${item.title}`, 15, y);
        doc.setFont('helvetica','normal'); doc.setFontSize(9);
        doc.text(`Alternative: ${item.resultData.alt} | Saving: ${item.resultData.cost_saving_pct || 35}%`, 15, y + 5);
        y += 15;
    });
    ...
    doc.text('Generated by EcoScan AI  |  ecoscan-pk.web.app', W/2, 282, {align:'center'});
    doc.save(`EcoScan_BatchReport_${getSessionId()}.pdf`);
};
...
const ScannerScreen = () => {
    ...
    const [globalImpact, setGlobalImpact] = useState({ total_scans: 1247, co2_saved: 4.2, cost_saved: 2100000 });
    ...
    React.useEffect(() => {
        ...
        // Fetch Live Impact
        fetch("https://ecoscan-backend-rxmp.onrender.com/impact")
            .then(r => r.json())
    ...
    const processScan = async (blob) => {
        ...
        try {
            ...
            reader.onloadend = () => {
                ...
                if (isBatchMode) {
                    setBatchItems(prev => [...prev, newEntry]);
                    setIsScanning(false);
                    // Update global impact locally for immediate feedback
                    setGlobalImpact(prev => ({
                        ...prev,
                        total_scans: prev.total_scans + 1,
                        co2_saved: prev.co2_saved + (data.carbon - data.alt_carbon),
                        cost_saved: prev.cost_saved + (data.cost * ((data.cost_saving_pct || 35) / 100))
                    }));
                } else {
    ...
    return (
        <div className="bg-background overflow-hidden h-screen">
            ...
            <main className="relative h-full w-full flex flex-col justify-center items-center pt-16 pb-24">
                ...
                <div className="relative z-10 w-full h-full flex flex-col items-center justify-center">
                    ...
                    <div className="absolute bottom-36 left-1/2 -translate-x-1/2 w-[90%] max-w-xs z-20">
                        <div className="bg-black/50 backdrop-blur-xl rounded-2xl border border-white/10 px-4 py-3 flex justify-around">
                            {[['🌍', globalImpact.total_scans, 'Total Scans'],
                              ['🌿', globalImpact.co2_saved.toFixed(1) + 't', 'CO₂ Saved'],
                              ['💰', '₨' + (globalImpact.cost_saved / 1000000).toFixed(1) + 'M', 'Cost Saved']].map(([icon, val, label]) => (
                ...
                {/* Batch Mode Toggle */}
                <div className="absolute top-24 right-6 z-20 flex flex-col items-center gap-1">
                    <button onClick={() => setIsBatchMode(!isBatchMode)} 
                        className={`w-12 h-12 rounded-full border-2 flex items-center justify-center transition-all ${isBatchMode ? 'bg-primary border-primary text-white' : 'bg-black/20 border-white/40 text-white'}`}>
                        <span className="material-symbols-outlined">{isBatchMode ? 'layers' : 'layers_clear'}</span>
                    </button>
                    <p className="text-white text-[10px] font-bold uppercase tracking-widest">{isBatchMode ? 'Batch ON' : 'Single'}</p>
                </div>

                {/* Batch Items List / Button */}
                {batchItems.length > 0 && (
                    <div className="absolute bottom-52 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-3 w-full px-6">
                        <div className="flex gap-2 overflow-x-auto max-w-full pb-2">
                            {batchItems.map((item, i) => (
                                <div key={i} className="w-12 h-12 rounded-lg border-2 border-primary overflow-hidden flex-shrink-0 animate-in fade-in zoom-in">
                                    <img src={item.img} className="w-full h-full object-cover" />
                                </div>
                            ))}
                        </div>
                        <button onClick={() => { downloadBatchCertificate(batchItems); setBatchItems([]); setIsBatchMode(false); }}
                            className="bg-primary text-white px-6 py-2.5 rounded-full font-bold text-sm shadow-xl flex items-center gap-2 active:scale-95 transition-transform">
                            <span className="material-symbols-outlined text-sm">assignment</span>
                            Generate Site Report ({batchItems.length})
                        </button>
                    </div>
                )}
